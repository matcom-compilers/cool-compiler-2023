...
class CoolParser:
    ...
    def p_class(self, p):
        ...
        if len(p) == 8:
            ...
            p[0] = ClassNode(id_=p[2], features=p[4], parent=None)
    ...
    def p_expression_conditional(self, p):
        ...
        """
        p[0] = ConditionalNode(
            if_expression=p[2], then_expression=p[4], else_expression=p[6]
        )
    ...
    def add_error(self, t):
        ...
        # print(t.line)
        self.errors.append(
            SyntacticError(message=f"{t.value}", line=t.line, column=t.col)
        )
