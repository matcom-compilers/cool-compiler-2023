...
_lr_productions = [
  ...
  ('program -> class SEMICOLON program','program',3,'p_program_1','parser_.py',8),
  ('program -> class SEMICOLON','program',2,'p_program_2','parser_.py',14),
  ('class -> CLASS TYPE inherits LBRACE feature_list RBRACE','class',6,'p_class','parser_.py',19),
  ('inherits -> INHERITS TYPE','inherits',2,'p_inherits','parser_.py',25),
  ('inherits -> epsilon','inherits',1,'p_inherits','parser_.py',26),
  ('feature_list -> feature SEMICOLON feature_list','feature_list',3,'p_feature_list','parser_.py',33),
  ('feature_list -> epsilon','feature_list',1,'p_feature_list','parser_.py',34),
  ('feature -> ID LPAREN formal_params RPAREN COLON TYPE LBRACE expr RBRACE','feature',9,'p_feature','parser_.py',45),
  ('feature -> attribute','feature',1,'p_feature','parser_.py',46),
  ('formal_params -> formal_list','formal_params',1,'p_formal_params_1','parser_.py',55),
  ('formal_params -> epsilon','formal_params',1,'p_formal_params_2','parser_.py',60),
  ('formal -> ID COLON TYPE','formal',3,'p_formal','parser_.py',65),
  ('formal_list -> formal formal_list_helper','formal_list',2,'p_formal_list','parser_.py',70),
  ('formal_list_helper -> COMMA formal formal_list_helper','formal_list_helper',3,'p_formal_list_helper','parser_.py',77),
  ('formal_list_helper -> epsilon','formal_list_helper',1,'p_formal_list_helper','parser_.py',78),
  ('expr_list_semicolon -> expr SEMICOLON expr_list_semicolon','expr_list_semicolon',3,'p_expr_list_semicolon','parser_.py',89),
  ('expr_list_semicolon -> expr SEMICOLON','expr_list_semicolon',2,'p_expr_list_semicolon','parser_.py',90),
  ('expr_params -> expr_list_comma','expr_params',1,'p_expr_params_1','parser_.py',100),
  ('expr_params -> epsilon','expr_params',1,'p_expr_params_2','parser_.py',105),
  ('expr_list_comma -> expr expr_list_comma_helper','expr_list_comma',2,'p_expr_list_comma','parser_.py',110),
  ('expr_list_comma_helper -> COMMA expr expr_list_comma_helper','expr_list_comma_helper',3,'p_expr_list_comma_helper','parser_.py',117),
  ('expr_list_comma_helper -> epsilon','expr_list_comma_helper',1,'p_expr_list_comma_helper','parser_.py',118),
  ('attribute -> ID COLON TYPE opt_expr_init','attribute',4,'p_attribute','parser_.py',128),
  ('let_var -> ID COLON TYPE opt_expr_init','let_var',4,'p_let_var','parser_.py',137),
  ('let_list -> let_var let_list_helper','let_list',2,'p_let_list','parser_.py',146),
  ('let_list_helper -> COMMA let_var let_list_helper','let_list_helper',3,'p_let_list_helper','parser_.py',153),
  ('let_list_helper -> epsilon','let_list_helper',1,'p_let_list_helper','parser_.py',154),
  ('opt_expr_init -> ASSIGN expr','opt_expr_init',2,'p_opt_expr_init','parser_.py',165),
  ('opt_expr_init -> epsilon','opt_expr_init',1,'p_opt_expr_init','parser_.py',166),
  ('case_var -> ID COLON TYPE','case_var',3,'p_expr_case_var','parser_.py',177),
  ('case_list -> case_var ARROW expr SEMICOLON case_list','case_list',5,'p_case_list','parser_.py',184),
  ('case_list -> case_var ARROW expr SEMICOLON','case_list',4,'p_case_list','parser_.py',185),
  ('expr -> ID ASSIGN expr','expr',3,'p_expr_assignment','parser_.py',197),
  ('expr -> expr CAST TYPE DOT ID LPAREN expr_params RPAREN','expr',8,'p_expr_dispatch_1','parser_.py',203),
  ('expr -> expr DOT ID LPAREN expr_params RPAREN','expr',6,'p_expr_dispatch_2','parser_.py',209),
  ('expr -> ID LPAREN expr_params RPAREN','expr',4,'p_expr_self_dispatch','parser_.py',215),
  ('expr -> IF expr THEN expr ELSE expr FI','expr',7,'p_expr_if','parser_.py',224),
  ('expr -> WHILE expr LOOP expr POOL','expr',5,'p_expr_while','parser_.py',230),
  ('expr -> LBRACE expr_list_semicolon RBRACE','expr',3,'p_expr_block','parser_.py',236),
  ('expr -> LET let_list IN expr','expr',4,'p_expr_let','parser_.py',244),
  ('expr -> CASE expr OF case_list ESAC','expr',5,'p_expr_case','parser_.py',250),
  ('expr -> NEW TYPE','expr',2,'p_expr_new','parser_.py',256),
  ('expr -> ISVOID expr','expr',2,'p_expr_unary_isvoid','parser_.py',262),
  ('expr -> INT_COMP expr','expr',2,'p_expr_unary_intcomp','parser_.py',268),
  ('expr -> NOT expr','expr',2,'p_expr_unary_not','parser_.py',274),
  ('expr -> expr PLUS expr','expr',3,'p_expr_binary_plus','parser_.py',280),
  ('expr -> expr MINUS expr','expr',3,'p_expr_binary_minus','parser_.py',286),
  ('expr -> expr MUL expr','expr',3,'p_expr_binary_mul','parser_.py',292),
  ('expr -> expr DIV expr','expr',3,'p_expr_binary_div','parser_.py',298),
  ('expr -> expr LESS expr','expr',3,'p_expr_binary_less','parser_.py',304),
  ('expr -> expr LESS_EQ expr','expr',3,'p_expr_binary_lesseq','parser_.py',310),
  ('expr -> expr EQ expr','expr',3,'p_expr_binary_eq','parser_.py',316),
  ('expr -> LPAREN expr RPAREN','expr',3,'p_expr_paren','parser_.py',322),
  ('expr -> ID','expr',1,'p_expr_id','parser_.py',328),
  ('expr -> INT','expr',1,'p_expr_int','parser_.py',333),
  ('expr -> STRING','expr',1,'p_expr_string','parser_.py',338),
  ('expr -> BOOL','expr',1,'p_expr_bool','parser_.py',343),
  ('epsilon -> <empty>','epsilon',0,'p_epsilon','parser_.py',349),
]
